...
def make_class(class_or_instance):
    """
    Converts an instance into it's original class.
...
def _get_current_registry(api=None):
    ...
    if api:
        ...
        overrides = {'basePath': urlparse.urlunparse(
            [proto] + list(urlparts[1:]))}
    ...
    for key, value in reg.iteritems():
        if key == 'models':
            ...
            log.debug("    apis:")
            for api_reg in value:
                for operation in api_reg['operations']:
                    log.debug("      %s, %s" % (api_reg['path'], operation['method']))
        else:
...
def add_model(model_class):
    ...
    if 'resource_fields' in dir(model_class):
        ...
        for arg in argspec.args[:required_args_count]:
            required.append(arg)
            # type: string for lack of better knowledge, until we add more
            # metadata
